    ...
    try:
        ...
        try:
            if vendor != 'huawei':
                ...
                shell = connect_ssh_shell(ip, username, password)
                time.sleep(5)
                logging.info(f"Connected to Huawei device for SSH upload...")
                time.sleep(1)
                ...
                shell.recv(2000)
                shell.send("copy flash:/vrpcfg.cfg flash:/restore.cfg\n")
                time.sleep(1)
                shell.recv(1000)
                shell.send("Y\n")
                time.sleep(1)
                shell.recv(1000)
                shell.send("system-view\n")
                shell.send("configuration replace flash:/restore.cfg\n")
                shell.recv(1000)
                logging.info(f"Replacing configuration with restore.cfg on Huawei device...")
                time.sleep(2)
                shell.send("Y\n")  # Confirm the replace prompt
                shell.recv(1000)
                logging.info(f"Configuration replaced successfully on Huawei device.")
                shell.send("save\n")
                
                shell.recv(2000)
                shell.send("Y\n")
                shell.recv(2000)
                logging.info(f"Configuration saved successfully on Huawei device.")
                time.sleep(1)
                shell.send("quit\n")
                shell.recv(1000)
                logging.info(f"Exiting system-view mode on Huawei device...")
                time.sleep(1)
                shell.send("exit\n")
                shell.recv(1000)
                # Close the shell
