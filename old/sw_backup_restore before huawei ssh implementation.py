...
# Configure logging
logging.basicConfig(
    filename=os.path.join(os.getcwd(),f'script_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),  # Log file in the current running location
...
def main():
    ...
    log_file_path = os.path.join(default_storage_path, f'script_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log')
    logging.basicConfig(
